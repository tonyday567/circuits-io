...
main :: IO ()
main = defaultMain $ testGroup "circuits-io Repl pipeline (mock)"
  [ testCase "mock repl starts and responds to a simple query (no extra noise)" $ do
      let cfg = (baseCfg ["--prompt=mock> ", "--delay=20", "--no-extra-noise"])
  ...
  , testCase "multiple commands with extra noise still sync correctly" $ do
      let cfg = (baseCfg ["--prompt=mock> ", "--delay=15"])
  ...
  , testCase "hanging prompt (no trailing newline) is still detected via improved readLines" $ do
      let cfg = (baseCfg ["--prompt=mock-hang> ", "--delay=10", "--hanging-prompt", "--no-extra-noise"])
  ...
  where
    ...
    cleanLogs cfg = mapM_ (\p -> whenM (doesFileExist p) (removeFile p))
